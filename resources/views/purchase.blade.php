...
@section('content')
    ...
    <div class="bg-white w-full max-w-4xl mx-auto rounded-2xl shadow-sm border border-gray-100 overflow-visible mt-6">
        <div class="bg-gradient-to-r from-blue-700 to-blue-800 p-6 text-center sm:text-left sm:px-8 rounded-t-2xl">
        ...
        <form action="/run-purchase" method="POST" class="p-6 sm:p-8 space-y-8" id="transaction-form">
            @csrf
            
            <div class="mb-4">
                <label class="block text-sm font-bold text-gray-700 mb-1">Transaction Date</label>
                <input type="date" name="voucher_date" value="{{ date('Y-m-d') }}" class="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-blue-500 outline-none">
            </div>

            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Select Party (Supplier / Customer)</label>
                <select name="party_id" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 text-gray-800 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer">
            ...
            <div class="bg-blue-50 border-2 border-blue-100 rounded-2xl shadow-sm overflow-visible">
                <div class="p-4 sm:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-blue-100">
                    <h3 class="font-bold text-gray-800 text-base flex items-center gap-2">
                        <span class="w-2 h-6 bg-blue-600 rounded-full"></span> ITEMS TO PURCHASE
                    </h3>
                    <div class="flex items-center gap-3 w-full sm:w-auto">
                        <label class="font-bold text-blue-800 text-xs tracking-wider uppercase whitespace-nowrap">Filter Group:</label>
                        <select id="group-filter" class="w-full sm:w-48 px-3 py-2 bg-white border border-blue-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm font-bold text-blue-900 cursor-pointer shadow-sm">
                ...
                <div class="p-5 sm:p-6 overflow-visible">
                    ...
                    <button type="button" onclick="addRow()" class="mt-5 w-full sm:w-auto text-sm font-semibold text-blue-700 bg-white hover:bg-blue-100 px-5 py-2.5 rounded-lg transition-colors flex justify-center items-center gap-2 border border-blue-200 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="invoice_number" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Optional reference">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Narration</label>
                    <input type="text" name="narration" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Rice purchase from supplier">
                </div>
            </div>

            <hr class="border-gray-200">
            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-4 rounded-xl shadow-md hover:shadow-lg transition-all flex justify-center items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Confirm Purchase
            </button>
        </form>
    </div>

    {{-- 🚨 ADDED: Hidden Template Row for extreme performance --}}
        ...
        <div class="transaction-row grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200 relative mt-4">
            <div>
                ...
                <div class="smart-select-wrapper relative">
                    ...
                    <input type="text" class="search-display w-full px-3 py-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none cursor-text font-bold text-gray-800 text-sm" placeholder="🔍 Type to search..." autocomplete="off" disabled>
                    ...
                    <div class="dropdown-menu absolute z-[100] w-full min-w-[250px] bg-white border border-gray-200 rounded-xl shadow-2xl mt-1 hidden max-h-60 overflow-y-auto top-full left-0">
                        @foreach($items as $item)
                            <div class="dropdown-item p-3 hover:bg-blue-50 cursor-pointer border-b border-gray-50 flex justify-between items-center" 
                                 data-value="{{ $item->id }}" 
                                ...
                                <div class="text-right">
                                    <div class="text-xs font-mono font-bold text-blue-600">Stock: {{ number_format($item->current_stock, 0) }}</div>
                                </div>
                            </div>
                        @endforeach
            ...
            <div>
                ...
                <input type="number" step="any" name="quantity[]" class="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. 10" required disabled>
            </div>
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Unit</label>
                <select name="unit_id[]" class="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer" required disabled>
            ...
            <div class="flex gap-2 items-end">
                <div class="flex-1">
                    ...
                    <input type="number" step="any" name="rate[]" class="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. 1500" required disabled>
                </div>
    ...
    <script>
        ...
        // Initialize the custom dropdown logic
        ...
        function addRow() {
            ...
            // Remove disabled attributes so the form can submit these values
