@extends('layouts.app')
@section('title', 'Edit Mill Production')
@section('content')
    <div class="bg-white w-full max-w-4xl mx-auto rounded-2xl shadow-xl overflow-hidden mb-20 mt-6">
        <div class="bg-green-600 p-4 flex justify-between items-center text-center md:text-left">
            <div>
                <h2 class="text-2xl font-bold text-white">Edit Production #VCH-{{ $voucher->id }}</h2>
                <p class="text-green-200 text-sm">Modify input raw materials and output yields</p>
            </div>
            <a href="/transactions" class="bg-green-800 text-white px-4 py-2 rounded-lg font-bold hover:bg-green-900 transition text-sm">Cancel</a>
        </div>

        <form action="/update-mill/{{ $voucher->id }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 border-l-4 border-l-red-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-3 raw-row">
                    <div>
                        ...
                        <select name="raw_item_id" id="raw_item" required class="w-full p-2 border border-gray-300 rounded focus:border-green-500 bg-white auto-calc-item">
                            ...
                            @foreach($rawMaterials as $item)
                                <option value="{{ $item->id }}" {{ ($rawMovement && $rawMovement->item_id == $item->id) ? 'selected' : '' }}>
                                    {{ $item->name }} (Stock: {{ number_format($item->current_stock, 0) }} KG)
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.01" name="raw_quantity" value="{{ $rawMovement->quantity ?? '' }}" required class="w-full p-2 border border-gray-300 rounded focus:border-green-500" placeholder="Qty">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="raw_rate" id="raw_rate" value="{{ $rawMovement->rate ?? '' }}" required class="w-full p-2 border border-gray-300 rounded focus:border-green-500 font-bold text-red-700 bg-red-50 auto-calc-rate" placeholder="৳">
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3 rice-row">
                        <div class="md:col-span-4">
                            ...
                            <select name="rice_item_id[]" required class="w-full p-2 border border-gray-300 rounded focus:border-green-500 bg-white auto-calc-item">
                                <option value="" disabled>Select Rice...</option>
                                @foreach($finishedGoods as $item)
                                    <option value="{{ $item->id }}" {{ $rice->item_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-3">
                            <label class="block text-xs font-bold text-gray-500 mb-1">Qty Produced</label>
                            <input type="number" step="0.01" name="rice_quantity[]" value="{{ $rice->quantity }}" required class="w-full p-2 border border-gray-300 rounded focus:border-green-500" placeholder="Qty">
                        </div>
                        ...
                        <div class="md:col-span-3 flex gap-2">
                            <div class="w-full">
                                ...
                                <input type="number" step="0.01" name="rice_rate[]" value="{{ $rice->rate }}" required class="w-full p-2 border border-gray-300 rounded focus:border-green-500 font-bold text-green-700 bg-green-50 auto-calc-rate" placeholder="৳">
                            </div>
                            <button type="button" onclick="this.parentElement.parentElement.remove()" class="bg-red-100 text-red-600 px-3 rounded font-bold hover:bg-red-200 h-[38px] mt-[20px]" title="Remove">X</button>
                        </div>
                    </div>
                    @empty
                    @endforelse
            ...
            <div class="bg-gray-50 p-4 rounded-xl border border-gray-200 border-l-4 border-l-orange-500">
                ...
                <div id="byproduct-container" class="space-y-3">
                    @foreach($byproductMovements as $byproduct)
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3 byproduct-row">
                        <div class="md:col-span-4">
                            ...
                            <select name="byproduct_item_id[]" class="w-full p-2 border border-gray-300 rounded focus:border-orange-500 bg-white auto-calc-item">
                                <option value="">Select Bran/Husk...</option>
                                @foreach($byproducts as $item)
                                    <option value="{{ $item->id }}" {{ $byproduct->item_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-3">
                            <label class="block text-xs font-bold text-gray-500 mb-1">Qty Produced</label>
                            <input type="number" step="0.01" name="byproduct_quantity[]" value="{{ $byproduct->quantity }}" class="w-full p-2 border border-gray-300 rounded focus:border-orange-500" placeholder="Qty">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold text-gray-500 mb-1">Unit</label>
                        ...
                        <div class="md:col-span-3 flex gap-2">
                            <div class="w-full">
                                <label class="block text-xs font-bold text-gray-500 mb-1">Value / Unit</label>
                                <input type="number" step="0.01" name="byproduct_rate[]" value="{{ $byproduct->rate }}" class="w-full p-2 border border-gray-300 rounded focus:border-orange-500 font-bold text-orange-700 bg-orange-50 auto-calc-rate" placeholder="৳">
                            </div>
                            <button type="button" onclick="this.parentElement.parentElement.remove()" class="bg-red-100 text-red-600 px-3 rounded font-bold hover:bg-red-200 h-[38px] mt-[20px]" title="Remove">X</button>
                        </div>
                    </div>
            ...
            <div>
                <input type="text" name="batch_number" value="{{ $voucher->reference_number }}" class="w-full p-3 border-2 border-gray-200 rounded-lg focus:border-green-500" placeholder="Batch / Reference Number (Optional)">
            </div>
            ...
            <button type="submit" class="w-full bg-green-600 text-white font-bold text-lg py-4 rounded-lg shadow-xl hover:bg-green-700 transition mt-4">
                Update & Recalculate Production Batch
            </button>
        </form>
    </div>
        ...
        function addRiceRow() {
            ...
            row.innerHTML = `
                ...
                <div class="md:col-span-3 flex gap-2">
                    ...
                    <button type="button" onclick="this.parentElement.parentElement.remove()" class="bg-red-100 text-red-600 px-3 rounded font-bold hover:bg-red-200 h-[38px]" title="Remove">X</button>
                </div>
            `;
            document.getElementById('rice-container').appendChild(row);
            attachRateCalculator(row); 
        }
