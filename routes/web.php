...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/mill', function () {
        ...
        foreach($finishedGoods->merge($byproducts) as $item) {
            $lastSaleRate = DB::table('inventory_movements')->join('vouchers', 'inventory_movements.voucher_id', '=', 'vouchers.id')
                ->where('inventory_movements.item_id', $item->id)->where('vouchers.voucher_type', 'Sales')
                ->orderBy('inventory_movements.id', 'desc')->value('inventory_movements.rate');
